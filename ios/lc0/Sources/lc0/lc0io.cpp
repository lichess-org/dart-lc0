...
// lc0 serialises its own output: StdoutUciResponder::SendRawResponses holds a
// mutex around the whole batch of lines it writes, and it is the only thing that
// writes from a search thread.
class FdOutBuf final: public std::streambuf {
 ...
 protected:
  ...
  // std::endl ends up here: this is what puts a finished line into the pipe.
...
struct Channel {
  ...
  Channel() {
      // Mirrors std::cin's tie to std::cout: a line written without a flush
      // still reaches the GUI, because the next read pushes it out.
      in.tie(&out);
  }
};
