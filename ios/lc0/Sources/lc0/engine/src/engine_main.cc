...
// Renamed from main(), and this file with it.
//
// Three reasons. Calling main() is not allowed in C++, and the plugin's shim has
// to call this to run the engine. A global main() cannot be merged into a host
// application's binary, which is what Swift Package Manager does with a plugin's
// object code -- it would collide with the app's own entry point. And SwiftPM
// decides a target is an executable rather than a library by looking for a file
// called main.cc, so the file's name has to go the same way as the symbol's.
