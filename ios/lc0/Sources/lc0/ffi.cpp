...
// Every read costs a syscall, an FFI crossing and a port message, so a smaller
// buffer means more of each than a searching engine leaves time for -- and a reader
// that falls behind fills the pipe, at which point the engine blocks in write()
// inside its UCI loop and stops answering commands, `quit` included.
...
// Diagnostics
//
// The engine runs on a thread this library does not own and can wedge in places
// that are invisible from Dart -- loading a network, initialising a backend, or
// joining its search threads on the way out. These few atomics are the only way
// to tell, from the outside, whether a start that never completed is stuck
// booting, stuck in the UCI loop, or stuck shutting down.
//
...
static std::atomic<int> g_phase{LC0_PHASE_IDLE};
static std::atomic<const char *> g_step{"idle"};
static std::atomic<long long> g_phase_since_ms{0};

// True between entering lc0_main() and returning from it. This is the re-entry
// guard: lc0 keeps its command line, its option registry and its backend
// factories in process globals, so a second concurrent run would race the first
// over the very objects it is tearing down.
...
//
// Only safe because init() refuses to run while an engine is alive: draining the
// input pipe would otherwise steal the running engine's commands.
...
int lc0_init()
{
  if (g_running.load(std::memory_order_acquire))
  {
    set_error(
        ...
        "It never returned from lc0_main(); this process cannot host another engine.",
    ...
    return LC0_INIT_ALREADY_RUNNING;
  }
  ...
  if (g_pipes_ready.load(std::memory_order_acquire))
  {
    // Reuse the existing pipes rather than creating a second pair: the previous
    // parent-side descriptors would simply be overwritten in the array, leaking
    // two per restart. Reusing them also keeps one stable channel for the
    // lifetime of the process, so the descriptors the engine's streams are
    // bound to always refer to the pipe this library is actually reading and
    // writing.
    drain_pipes();
    set_phase(LC0_PHASE_INITIALIZED, "pipes_reused");
    return 0;
  }

  set_phase(LC0_PHASE_INITIALIZING, "creating_pipes");
  ...
  if (pipe(pipes[PARENT_READ_PIPE]) != 0)
  {
    ...
    set_phase(LC0_PHASE_FAILED, "pipe_failed");
    return LC0_INIT_PIPE_FAILED;
  }
  ...
  if (flags < 0 || fcntl(PARENT_WRITE_FD, F_SETFL, flags | O_NONBLOCK) < 0)
  {
    ...
    close_pipe_pair(PARENT_READ_PIPE);
    close_pipe_pair(PARENT_WRITE_PIPE);
    set_phase(LC0_PHASE_FAILED, "fcntl_failed");
    return LC0_INIT_FCNTL_FAILED;
  }

  g_pipes_ready.store(true, std::memory_order_release);
  set_phase(LC0_PHASE_INITIALIZED, "pipes_created");
  return 0;
}
...
int lc0_main()
{
  if (!g_pipes_ready.load(std::memory_order_acquire))
  {
    set_error("main: called before a successful lc0_init()");
    set_phase(LC0_PHASE_FAILED, "not_initialized");
    return LC0_MAIN_NOT_INITIALIZED;
  }
  ...
  if (!g_running.compare_exchange_strong(expected, true))
  {
    set_error(
        ...
        g_step.load(std::memory_order_relaxed),
        steady_now_ms() - g_phase_since_ms.load(std::memory_order_relaxed));
    return LC0_MAIN_ALREADY_RUNNING;
  }

  set_phase(LC0_PHASE_REDIRECTING, "bind_streams");
  ...
  // took over the whole process's standard descriptors: no other engine could be
  // resident at the same time, and the host application lost its own stdout for
  // as long as lc0 was running.
  if (!lc0io::bind(CHILD_READ_FD, CHILD_WRITE_FD))
  {
    set_error("main: could not bind the engine's streams to the pipe descriptors");
    set_phase(LC0_PHASE_FAILED, "bind_failed");
    signal_quit();
    g_running.store(false, std::memory_order_release);
    return LC0_MAIN_BIND_FAILED;
  }
  ...
  catch (const std::exception &e)
  {
    ...
    set_phase(LC0_PHASE_FAILED, "engine_threw");
    signal_quit();
    g_running.store(false, std::memory_order_release);
...
ssize_t lc0_stdin_write(char *data)
{
  if (!g_pipes_ready.load(std::memory_order_acquire))
  {
    set_error("stdin_write: called before a successful lc0_init()");
    return LC0_WRITE_NOT_INITIALIZED;
  }
  ...
  while (written < length)
  {
    ...
    if (count < 0 && (errno == EAGAIN || errno == EWOULDBLOCK))
    {
      if (steady_now_ms() >= deadline)
      {
        ...
        return written > 0 ? LC0_WRITE_PARTIAL : LC0_WRITE_PIPE_FULL;
      }
      usleep(2000);
      continue;
    }
    ...
    return written > 0 ? LC0_WRITE_PARTIAL : LC0_WRITE_FAILED;
  }
...
char *lc0_stdout_read()
{
  if (!g_pipes_ready.load(std::memory_order_acquire))
  {
    set_error("stdout_read: called before a successful lc0_init()");
    return NULL;
  }
...
void lc0_set_phase(int phase, const char *step)
{
  set_phase(phase, step);
}
